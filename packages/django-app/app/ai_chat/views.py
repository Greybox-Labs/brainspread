...
from .models import ChatSession, ChatMessage, AIProvider, UserAISettings, UserProviderConfig
...
def ai_settings(request):
    ...
    try:
        ...
        if user_settings:
            current_provider = user_settings.provider.name if user_settings.provider else None
...
def update_ai_settings(request):
    ...
    try:
        ...
        provider_configs = request.data.get("provider_configs", {})  # Dict of provider configs
        ...
        for provider_name, config_data in provider_configs.items():
            try:
                ...
                if not created:
                    provider_config.is_enabled = config_data.get("is_enabled", provider_config.is_enabled)
                    provider_config.enabled_models = config_data.get("enabled_models", provider_config.enabled_models)
                    provider_config.save()
                    
            except AIProvider.DoesNotExist:
                logger.warning(f"Provider '{provider_name}' not found when updating config")
                continue
        ...
        for provider_name, api_key in api_keys.items():
            try:
                ...
                logger.warning(f"Provider '{provider_name}' not found when updating API key")
                continue
        
        return Response({"success": True, "data": {"message": "AI settings updated successfully"}})
