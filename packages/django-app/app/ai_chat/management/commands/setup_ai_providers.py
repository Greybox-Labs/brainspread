...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"AIProvider 'Anthropic' {'created' if created else 'already exists'} (ID: {anthropic_provider.id})")
        )

        openai_provider, created = AIProvider.objects.get_or_create(
            name='OpenAI',
            defaults={'base_url': 'https://api.openai.com'}
        )
        self.stdout.write(
            self.style.SUCCESS(f"AIProvider 'OpenAI' {'created' if created else 'already exists'} (ID: {openai_provider.id})")
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"UserProviderConfig for Anthropic {'created' if created else 'already exists'}")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(f"UserProviderConfig for OpenAI {'created' if created else 'already exists'}")
            )
                
            self.stdout.write(
                self.style.SUCCESS("Setup complete! Users can now configure API keys and models in the AI Settings.")
            )
            
        except User.DoesNotExist:
            self.stdout.write(
                self.style.ERROR(f"User with email '{user_email}' not found. Please create a user first or use --user-email option.")
            )