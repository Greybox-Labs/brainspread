...
class ApiService {
    ...
    async request(url, options = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(data.detail || data.errors?.non_field_errors?.[0] || 'Request failed');
            }
    ...
    async getEntry(entryDate) {
        return await this.request(`/journals/api/entries/get/?entry_date=${entryDate}`);
    }

    async getEntries(limit = 10, offset = 0) {
        return await this.request(`/journals/api/entries/list/?limit=${limit}&offset=${offset}`);
    }
    ...
    async getPages(publishedOnly = true, limit = 10, offset = 0) {
        return await this.request(`/journals/api/pages/list/?published_only=${publishedOnly}&limit=${limit}&offset=${offset}`);
    }
